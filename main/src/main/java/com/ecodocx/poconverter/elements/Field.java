...
public class Field extends BaseElement {
    ...
    String fontName;
    int fontSize;
    private String alignment = Text.textAlignment.LEFT.name();
    boolean wrapKeepSpace = false;
    private int textStyle;

    public Field(BaseElement parent, String line) {
        super(parent, line);
        ...
        if (words.contains("Key") && words.contains("Center") ) {
            alignment = Text.textAlignment.CENTER.toString();
        } else if (words.contains("Key") && words.contains("Right")){
            alignment = Text.textAlignment.RIGHT.toString();
        }
    ...
    public String getXinch() {
        if (getAlignment().equals(Text.textAlignment.CENTER.toString())) {
            return String.valueOf((float) (x/25.4) - getTextWidthInch()/2);
        } else if (getAlignment().equals(Text.textAlignment.RIGHT.toString())){
    ...
    public float getTextWidthInch() {
        if (autosize) {
            ...
            float str = (float) (font.getStringBounds(fieldName, frc).getWidth() / 72 + 0.02);
            return str;
        } else
    ...
    public float getTextHeightInch() {
        if (autosize) {
            ...
            FontRenderContext frc = new FontRenderContext(affinetransform, true, true);
            Font font = new Font(getFont(), getTextStyle(), fontSize);
            return (float) (font.getStringBounds(fieldName, frc).getHeight() / 72);
        }
        return (float) (h/25.4);
